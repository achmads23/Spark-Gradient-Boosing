...
public class TugasAkhir {
    ...
    public static void TemporaryFunction() throws IOException{
        ...
        /** End Cek HSB **/
            ...
            image = new Brigthness().Brigthness(image,50);
            output = new WriteImage().WriteImage(image, "Brigthness");
    }
    
    public TugasAkhir(String filepath){
        try {
            BufferedImage image;
            String output,fileinput;
            BufferedImage originalimage, imageori,imagemorphology;
            image = new ReadImage().ReadImage(filepath);
            output = new WriteImage().WriteImage(image, "File Input");
//            image = Scalr.resize(image, Method.SPEED, 500, OP_BRIGHTER);
//            output = new WriteImage().WriteImage(image, "Resize");
            originalimage = image;
            imageori = image;
            ...
            image = new Opening().Opening(image,2,StructuringElement.STRUCTURING_ELEMENT_SHAPE.VERTICAL_LINE,StructuringElement.STRUCTURING_ELEMENT_SHAPE.ROUND,1,1);
            output = new WriteImage().WriteImage(image, "Opening");
            
            image = new Erosion().Erosion(image,2,StructuringElement.STRUCTURING_ELEMENT_SHAPE.ROUND);
            output = new WriteImage().WriteImage(image, "Erosion");
            
//            imagemorphology = image;
...
//                writer.print(fitur.get(x).get(0) + ". ");
//                for (int y = 0; y < 5;y++ ){
...
//            }
//            
//            output = new WriteImage().WriteImage(originalimage, "Output");
            ProcessedImage processed = new ProcessedImage(originalimage, output);
